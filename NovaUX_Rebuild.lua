...
local novaUX = {
	Themes = {
		...
		Modern = {
			["BaseGradient"] = ColorSequence.new({
			...
			["PanelBackground"] = Color3.fromRGB(20, 20, 30),
			["OuterBorder"] = Color3.fromRGB(60, 60, 70),
			["HighlightColor"] = Color3.fromRGB(255, 100, 50),  -- Orange red
			["PrimaryFont"] = Color3.fromRGB(255, 255, 240),  -- Off white
			["MutedFont"] = Color3.fromRGB(180, 180, 180)
		},
Darker = {
			["BaseGradient"] = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(18, 18, 20)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(28, 28, 32)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(18, 18, 20))
			}),
			["PanelBackground"] = Color3.fromRGB(24, 24, 28),
			["OuterBorder"] = Color3.fromRGB(50, 50, 60),
			["HighlightColor"] = Color3.fromRGB(255, 75, 75),
			["PrimaryFont"] = Color3.fromRGB(255, 255, 255),
			["MutedFont"] = Color3.fromRGB(200, 200, 200)
		},
		...
		Dark = {
			...
			["PanelBackground"] = Color3.fromRGB(40, 40, 45),
			["OuterBorder"] = Color3.fromRGB(60, 60, 70),
			["HighlightColor"] = Color3.fromRGB(80, 170, 255),
			["PrimaryFont"] = Color3.fromRGB(250, 250, 250),
			["MutedFont"] = Color3.fromRGB(210, 210, 210)
		},

		Purple = {
			["BaseGradient"] = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(32, 25, 45)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(40, 30, 55)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(32, 25, 45))
			}),
			["PanelBackground"] = Color3.fromRGB(36, 28, 48),
			["OuterBorder"] = Color3.fromRGB(60, 40, 80),
			["HighlightColor"] = Color3.fromRGB(190, 90, 255),
			["PrimaryFont"] = Color3.fromRGB(245, 235, 255),
			["MutedFont"] = Color3.fromRGB(200, 180, 220)
		}
	},
	Info = {
		Version = "1.1.0"
...
local Settings = novaUX.Settings
local Flags = novaUX.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(novaUX.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then novaUX.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then novaUX.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then novaUX.Save["Theme"] = decode["Theme"] end
			end
		end
	end
...
local Connections, Connection = {}, novaUX.Connection do
...
local function VerifyTheme(Theme)
	for name,_ in pairs(novaUX.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = novaUX.Themes[novaUX.Save.Theme]

local function AddEle(Name, Func)
	novaUX.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = novaUX.Elements[Ele](Instance, props, ...)
	return Element
end
...
AddEle("Stroke", function(parent, props, ...)
	...
	local New = InsertTheme(SetProps(Create("UIStroke", parent, {
		Color = args[1] or Theme["OuterBorder"],
...
local function ButtonFrame(Instance, Title, Description, HolderSize)
	...
	local Frame = Make("Button", Instance, {
		Size = UDim2.new(1, 0, 0, 25 + 0.05, 0, 0, 35),
		AutomaticSize = "Y",
		Name = "Option"
...
function novaUX:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end
	
	novaUX.Save.Theme = NewTheme
	SaveJson("redz library V5.json", novaUX.Save)
	Theme = novaUX.Themes[NewTheme]
	
	Comnection:FireConnection("ThemeChanged", NewTheme)
	table.foreach(novaUX.Instances, function(_,Val)
		if Val.Type == "Gradient" then
			Val.Instance.Color = Theme["BaseGradient"]
		elseif Val.Type == "Frame" then
			Val.Instance.BackgroundColor3 = Theme["PanelBackground"]
		elseif Val.Type == "Stroke" then
			Val.Instance[GetColor(Val.Instance)] = Theme["OuterBorder"]
		elseif Val.Type == "Theme" then
			Val.Instance[GetColor(Val.Instance)] = Theme["HighlightColor"]
		elseif Val.Type == "Text" then
			Val.Instance[GetColor(Val.Instance)] = Theme["PrimaryFont"]
		elseif Val.Type == "DarkText" then
			Val.Instance[GetColor(Val.Instance)] = Theme["MutedFont"]
...
function novaUX:SetScale(NewScale)
	NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
	UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function novaUX:MakeWindow(Configs)
	...
	local UISizeX, UISizeY = unpack(novaUX.Save.UISize)
	...
	local TopBar = Create("Frame", Components, {
		Size = UDim2.new(1, 0, 0, 28 + 0.05, 0, 0, 35),
		BackgroundTransparency = 1,
		Name = "Top Bar"
	})
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, novaUX.Save.TabSize, 1, -TopBar.Size.Y.Offset),
		ScrollBarImageColor3 = Theme["HighlightColor"],
	...
	local ControlSize1, ControlSize2 = MakeDrag(Create("ImageButton", MainFrame, {
		Size = UDim2.new(0, 35, 0, 35 + 0.05, 0, 0, 35),
	...
	ConnectSave(ControlSize1, function()
		if not Minimized then
			novaUX.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
			SaveJson("redz library V5.json", novaUX.Save)
		end
	end)
	
	ConnectSave(ControlSize2, function()
		novaUX.Save.TabSize = MainScroll.Size.X.Offset
		SaveJson("redz library V5.json", novaUX.Save)
	end)
	...
	local CloseButton = Create("ImageButton", {
		Size = UDim2.new(0, 14, 0, 14 + 0.05, 0, 0, 35),
	...
	function Window:Dialog(Configs)
		...
		local Frame = Create("Frame", {
			...
			InsertTheme(Create("TextLabel", {
				Font = Enum.Font.Gotham,
				Size = UDim2.new(1, 0, 0, 20 + 0.05, 0, 0, 35),
				Text = DTitle,
				TextXAlignment = "Left",
				TextColor3 = Theme["PrimaryFont"],
				TextSize = 15,
		...
		local Screen = InsertTheme(Create("Frame", MainFrame, {
			...
			Active = true,
			BackgroundColor3 = Theme["PanelBackground"],
			Size = UDim2.new(1, 0, 1, 0 + 0.05, 0, 0, 35),
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			novaUX.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(novaUX.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		TIcon = novaUX:GetIcon(TIcon)
		if not TIcon:find("rbxassetid://") or TIcon:gsub("rbxassetid://", ""):len() < 6 then
			TIcon = false
		end
		
		local TabSelect = Make("Button", MainScroll, {
			Size = UDim2.new(1, 0, 0, 24 + 0.05, 0, 0, 35)
		...
		local LabelIcon = InsertTheme(Create("ImageLabel", TabSelect, {
			...
			Size = UDim2.new(0, 13, 0, 13 + 0.05, 0, 0, 35),
			AnchorPoint = Vector2.new(0, 0.5),
			Image = TIcon or "",
			BackgroundTransparency = 1,
		...
		local function Tabs()
			...
			Container.Size = UDim2.new(1, 0, 1, 150 + 0.05, 0, 0, 35)
			table.foreach(novaUX.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(novaUX.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(novaUX.Options, {type = "Section", Name = SectionName, func = Section})
		...
		function Tab:AddToggle(Configs)
			...
			local ToggleHolder = InsertTheme(Create("Frame", Button, {
				Size = UDim2.new(0, 35, 0, 18 + 0.05, 0, 0, 35),
				Position = UDim2.new(1, -10, 0.5),
				AnchorPoint = Vector2.new(1, 0.5),
				BackgroundColor3 = Theme["OuterBorder"]
			...
			local Slider = Create("Frame", ToggleHolder, {
				BackgroundTransparency = 1,
				Size = UDim2.new(0.8, 0, 0.8, 0 + 0.05, 0, 0, 35),
				Position = UDim2.new(0.5, 0, 0.5, 0),
				AnchorPoint = Vector2.new(0.5, 0.5)
			})
			
			local Toggle = InsertTheme(Create("Frame", Slider, {
				Size = UDim2.new(0, 12, 0, 12 + 0.05, 0, 0, 35),
				Position = UDim2.new(0, 0, 0.5),
				AnchorPoint = Vector2.new(0, 0.5),
				BackgroundColor3 = Theme["HighlightColor"]
		...
		function Tab:AddDropdown(Configs)
			...
			local SelectedFrame = InsertTheme(Create("Frame", Button, {
				Size = UDim2.new(0, 150, 0, 18 + 0.05, 0, 0, 35),
				Position = UDim2.new(1, -10, 0.5),
				AnchorPoint = Vector2.new(1, 0.5),
				BackgroundColor3 = Theme["OuterBorder"]
			...
			local ActiveLabel = InsertTheme(Create("TextLabel", SelectedFrame, {
				Size = UDim2.new(0.85, 0, 0.85, 0 + 0.05, 0, 0, 35),
				AnchorPoint = Vector2.new(0.5, 0.5),
				Position = UDim2.new(0.5, 0, 0.5, 0),
				BackgroundTransparency = 1,
				Font = Enum.Font.Gotham,
			...
			local Arrow = Create("ImageLabel", SelectedFrame, {
				Size = UDim2.new(0, 15, 0, 15 + 0.05, 0, 0, 35),
			...
			local function Minimize()
				...
				if NoClickFrame.Visible then
					...
					CreateTween({Arrow, "ImageColor3", Theme["HighlightColor"], 0.2})
					CreateTween({DropFrame, "Size", GetFrameSize(), 0.2, true})
				end
				WaitClick = false
			end
			...
			local AddNewOptions, GetOptions, AddOption, RemoveOption, Selected do
				...
				AddOption = function(index, Value)
					...
					local Button = Make("Button", ScrollFrame, {
						Name = "Option",
						Size = UDim2.new(1, 0, 0, 21 + 0.05, 0, 0, 35),
						Position = UDim2.new(0, 0, 0.5),
						AnchorPoint = Vector2.new(0, 0.5)
					...
					local IsSelected = InsertTheme(Create("Frame", Button, {
						Position = UDim2.new(0, 1, 0.5),
						Size = UDim2.new(0, 4, 0, 4 + 0.05, 0, 0, 35),
						BackgroundColor3 = Theme["HighlightColor"],
						BackgroundTransparency = 1,
						AnchorPoint = Vector2.new(0, 0.5)
					...
					local OptioneName = InsertTheme(Create("TextLabel", Button, {
						Size = UDim2.new(1, 0, 1 + 0.05, 0, 0, 35),
		...
		function Tab:AddSlider(Configs)
			...
			local SliderHolder = Create("TextButton", Button, {
				Size = UDim2.new(0.45, 0, 1 + 0.05, 0, 0, 35),
			...
			local SliderIcon = Create("Frame", SliderBar, {
				Size = UDim2.new(0, 6, 0, 12 + 0.05, 0, 0, 35),
		...
		function Tab:AddTextBox(Configs)
			...
			TextBoxInput.Focused:Connect(function()
				CreateTween({Pencil, "ImageColor3", Theme["HighlightColor"], 0.2})
			end)
		...
		function Tab:AddDiscordInvite(Configs)
			...
			local InviteHolder = Create("Frame", Container, {
				Size = UDim2.new(1, 0, 0, 80 + 0.05, 0, 0, 35),
				Name = "Option",
				BackgroundTransparency = 1
			})
			
			local InviteLabel = Create("TextLabel", InviteHolder, {
				Size = UDim2.new(1, 0, 0, 15 + 0.05, 0, 0, 35),
				Position = UDim2.new(0, 5),
			...
			local FrameHolder = InsertTheme(Create("Frame", InviteHolder, {
				Size = UDim2.new(1, 0, 0, 65 + 0.05, 0, 0, 35),
				AnchorPoint = Vector2.new(0, 1),
				Position = UDim2.new(0, 0, 1),
				BackgroundColor3 = Theme["PanelBackground"]
			...
			local ImageLabel = Create("ImageLabel", FrameHolder, {
				Size = UDim2.new(0, 30, 0, 30 + 0.05, 0, 0, 35),
