	...
	Themes = {
		...
		NovaTheme = {
			["PrimaryGradient"] = ColorSequence.new({
				...
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(30, 20, 50)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 10, 30))
			}),
			["Background"] = Color3.fromRGB(25, 15, 40),
			["Border"] = Color3.fromRGB(70, 50, 90),
			["Accent"] = Color3.fromRGB(120, 80, 255),  -- Orange red
			["Text"] = Color3.fromRGB(220, 220, 255),  -- Off white
			["SecondaryText"] = Color3.fromRGB(150, 180, 180)
		},
Darker = {
			["PrimaryGradient"] = ColorSequence.new({
		...
		Dark = {
			...
			["SecondaryText"] = Color3.fromRGB(210, 210, 210)
		},
...
local Flags = novaUI.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(novaUI.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then novaUI.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then novaUI.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then novaUI.Save["Theme"] = decode["Theme"] end
			end
		end
	end
...
local function VerifyTheme(Theme)
	for name,_ in pairs(novaUI.Themes) do
		if name == Theme then
			return true
		end
	end
...
local function Make(Ele, Instance, props, ...)
	local Element = novaUI.Elements[Ele](Instance, props, ...)
	return Element
end
...
function novaUI:SetTheme(NewTheme)
	...
	novaUI.Save.Theme = NewTheme
	SaveJson("redz library V5.json", novaUI.Save)
	Theme = novaUI.Themes[NewTheme]
	
	Comnection:FireConnection("ThemeChanged", NewTheme)
	table.foreach(novaUI.Instances, function(_,Val)
		if Val.Type == "Gradient" then
			Val.Instance.Color = Theme["PrimaryGradient"]
		elseif Val.Type == "Frame" then
			Val.Instance.BackgroundColor3 = Theme["Background"]
		elseif Val.Type == "Stroke" then
			Val.Instance[GetColor(Val.Instance)] = Theme["Border"]
		elseif Val.Type == "Theme" then
			Val.Instance[GetColor(Val.Instance)] = Theme["Accent"]
		elseif Val.Type == "Text" then
			Val.Instance[GetColor(Val.Instance)] = Theme["Text"]
		elseif Val.Type == "DarkText" then
			Val.Instance[GetColor(Val.Instance)] = Theme["SecondaryText"]
...
function novaUI:MakeWindow(Configs)
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			novaUI.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(novaUI.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		local function Tabs()
			...
			table.foreach(novaUI.Tabs, function(_,Tab)
		...
		function Tab:AddDropdown(Configs)
			...
			local function Minimize()
				...
				if NoClickFrame.Visible then
					...
					CreateTween({Arrow, "ImageColor3", Theme["Accent"], 0.2})
					CreateTween({DropFrame, "Size", GetFrameSize(), 0.2, true})
				end
				WaitClick = false
			end
		...
		function Tab:AddTextBox(Configs)
			...
			TextBoxInput.Focused:Connect(function()
				CreateTween({Pencil, "ImageColor3", Theme["Accent"], 0.2})
			end)
